...
@torch.no_grad()        
def evaluation_transformer(out_dir, val_loader, net, trans, diff, logger, writer, nb_iter, best_fid, best_iter, best_div, best_top1, best_top2, best_top3, best_matching, clip_model, eval_wrapper, draw = True, save = True, savegif=False) : 

    # TODO
    # evaluation_transformer와 evaluation_transforemr_test 둘 다
    # diff sampling 결과를 trans output -> decoder와 합치기
    ...
    if fid < best_fid : 
        ...
        best_fid, best_iter = fid, nb_iter
        if save:
            torch.save({'trans' : trans.state_dict(), 'diff' : diff.state_dict()}, os.path.join(out_dir, 'net_best_fid.pth'))
    ...
    if save:
        torch.save({'trans' : trans.state_dict(), 'diff' : diff.state_dict()}, os.path.join(out_dir, 'net_last.pth'))
...
@torch.no_grad()        
def evaluation_transformer_test(out_dir, val_loader, net, trans, diff, logger, writer, nb_iter, best_fid, best_iter, best_div, best_top1, best_top2, best_top3, best_matching, best_multi, clip_model, eval_wrapper, draw = True, save = True, savegif=False, savenpy=False) : 
