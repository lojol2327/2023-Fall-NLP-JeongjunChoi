...
class Text2MotionDataset(data.Dataset):
    def __init__(self, dataset_name, feat_bias = 5, unit_length = 4, codebook_size = 1024, tokenizer_name=None):
        ...
        if dataset_name == 't2m':
            ...
            self.max_token_length = 26 if unit_length == 8 else 51
            dim_pose = 263
    ...
    def __getitem__(self, item):
        ...
        if m_tokens_len+1 < self.max_token_length:
            m_tokens = np.concatenate([m_tokens, np.ones((1), dtype=int) * self.mot_end_idx, np.ones((self.max_token_length-1-m_tokens_len), dtype=int) * self.mot_pad_idx], axis=0)
        else:
            m_tokens = np.concatenate([m_tokens, np.ones((1), dtype=int) * self.mot_end_idx], axis=0)

        return motion, caption, m_tokens.reshape(-1), m_tokens_len
