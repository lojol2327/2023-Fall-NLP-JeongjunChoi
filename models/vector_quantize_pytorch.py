...
class EuclideanCodebook(nn.Module):
    ...
    def forward(
        ...
        'embed_onehot -> [h BxT D]: [1 4096 512]'
        code_count = embed_onehot.sum(dim=1).squeeze()
        prob = code_count / torch.sum(code_count)
        perplexity = torch.exp(-torch.sum(prob * torch.log(prob + 1e-7)))
        
        embed_ind = unpack_one(embed_ind, ps, 'h *')

        # Actually, 'embed_ind' is the result of quantization
        # and 'quantize' is the result of dequantization
        if self.training:
...
class VectorQuantize(nn.Module):
    ...
    def forward(
        ...
        quantize, embed_ind, distances, perplexity = self._codebook(x, **codebook_forward_kwargs)
